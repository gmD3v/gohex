...
import (
	...
	"github.com/gmd3v/gohex/internal/platform/storage/storagemocks"
...
func TestHandler_Create(t *testing.T) {
	...
	t.Run("given a valid request it returns 201", func(t *testing.T) {
		...
		b, err := json.Marshal(createCourseReq)
		require.NoError(t, err) // if error is not nil, it will stop the execution of the test
		...
		assert.Equal(t, http.StatusCreated, res.StatusCode) // the expected status code is 201, not will stop the execution of the test
	})
}
